...
namespace Primer_Obligatorio
{
    class Program
    {
        ...
        public static void Apuesta(ref int ultimoapostado, ref string[] nombres, ref int[,] apuestas)
        {
            ...
            for(int i=0; i<5;i++)
            {
                Console.WriteLine("Ingrese el {0}º valor de la apuesta", i+1);
                bool esnumero = Int32.TryParse(Console.ReadLine(), out apuestas[ultimoapostado,i]);
                if (!esnumero)
                {
                    Console.Write("La opción ingresada no es válida.\nIngrese nuevamente el {0}º valor de la apuesta", i+1);
                    Console.ReadLine();
                }
            }
            ultimoapostado++;
